...
export const CreateAPromotionRule = (data: { promotion_rule: PromotionActionRule }, paramsObj?: { include?: string }): Promise<AxiosResponse> => {
...
export const ReturnAPromotionRule = (id: string, paramsObj?: { include?: string }): Promise<AxiosResponse> => {
    const params = new URLSearchParams();
    if (paramsObj?.include) params.append("include", paramsObj?.include);
    return api.get(`/platform/promotion_rules/${id}?${decodeURIComponent(params.toString())}`);
}
export const UpdateAPromotionRule = (data: { promotion_rule: PromotionActionRuleUpdate }, id: string, paramsObj?: { include?: string }): Promise<AxiosResponse> => {
    const params = new URLSearchParams();
    if (paramsObj?.include) params.append("include", paramsObj?.include);
